...
class CategoriesDetailsAPIVIew(APIView):
...
class SubCategoriesDetailsAPIVIew(APIView):
    def get(self, request, subCategory_id):
        subCategory = get_object_or_404(SubCategory, id=subCategory_id)
        serializers = SubCategorySerializer(subCategory)
        return Response(serializers.data)
...
class ProductsAPIView(APIView):
    ...
    def get(self, request):
        ...
        if brand_search is not None:
            products = products.filter(brand__isontains=brand_search)
